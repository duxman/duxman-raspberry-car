...
public class Blobs
{
    ...
    public String PrintRegionData()
    {
        return PrintRegionData(0, MaxLabel);
    }

    public String PrintRegionData(int Label0, int Label1)
    {
        ...
        for (int Label = Label0; Label <= Label1; Label++)
        {
            ...
            rtn.append(Str1).append(Str2).append(Str3).append(Str4).append("\r\n");                        
        }        
    ...
    public static int NextRegion(int Parent, int Color, double MinArea, double MaxArea, int Label)
    {
        ...
        for (i = Label; i <= MaxLabel; i++)
        {
            ...
            if (DParent >= 0 && DParent != ThisParent)
            {
                continue;
            }
            if (DColor >= 0 && DColor != ThisColor)
            {
                continue;
            }
            if (Region[BLOBAREA] < MinArea || Region[BLOBAREA] > MaxArea)
            {
                continue;
            }
        ...
        if (i > MaxLabel)
        {
            i = -1;    // Use -1 to flag that there was no match
        }
        return i;
    }
            ...
            int MinArea)                        // minimum region area
    {
        ...
        for (int irow = Row0; irow < Row0 + Rows; irow++)  // index within Src
        {
            ...
            for (int icol = Col0; icol < Col0 + Cols; icol++)  // index within Src
            {
                ...
                LabelA = LabelB = LabelC = LabelD = 0;
                ColorD = (int) getPixelValue(SrcMat, jrow, jcol);       // fetch color of cell

                if (jrow == 0 || jcol == 0)  // first column or row
                {
                    if (jcol > 0)
                    {
                        ColorC = (int) getPixelValue(SrcMat, jrow, jcol - 1);
                        LabelC = LabelMat[jrow][jcol - 1];
                    }
                    if (jrow > 0)
                    {
                        ColorB = (int) getPixelValue(SrcMat, jrow - 1, jcol);
                        LabelB = LabelMat[jrow - 1][jcol];
                    }
                }
                else
                {
                    ColorA = (int) getPixelValue(SrcMat, jrow-1,jcol-1); 
                    if (ColorA > 0)
                    {
                        ColorA = 1;
                    }
                ...
                if (Case == 1)
                {
                    OldRegion(LabelC, -1, -1);
                }
                else if (Case == 2 || Case == 3)
                {
                    OldRegion(LabelC, LabelB, LabelC);
                }
                ...
                {
                    OldRegion(LabelB, LabelB, LabelC);
                }
        ...
        for (int Label = NewMaxLabel + 1; Label <= MaxLabel; Label++)
        {
            ResetRegion(Label);
        }
    ...
    private int getPixelValue(Mat src, int row, int col, int div)
    {
        byte[] ColorBuffer = { 0, 0, 0 };
             
        src.get(row, col, ColorBuffer);        
        ...
        //int rgb = (r*6/256)*36 + (g*6/256)*6 + (b*6/256);
        return rgb;
    }
    
    private int getPixelValue(Mat src, int row, int col)
    {
        return getPixelValue (src, row, col, 8);        
    }
}
